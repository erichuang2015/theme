...
(function( $ )
{
	...
	function Plugin( elem, options )
	{
		...
		this.$form.on( 'change', 'input[type="checkbox"], input[type="radio"]', function( event )
		{
		...
		this.$form.on( 'change', ':input.autoload', function( event )
		{
			// Load
			_this.load();
		});
	...
	Plugin.prototype.load = function( options ) 
	{
		...
		// Set page
		this.$form.find( ':input[name="paged"]' ).val( options.page );

		// Get fields
		var $fields = this.$form.find( ':input:not([disabled])' );
		...
		$.ajax(
		{
			...
			success : function( response, textStatus, jqXHR )
			{
				...
				// Set content
				this.$content.html( this.response.result );
